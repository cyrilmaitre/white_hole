...
void StationScreen::update( sf::Event p_event )
{
	...
	if( p_event.type == sf::Event::KeyPressed && p_event.key.code == sf::Keyboard::S)
		UserInterface::mUserInterface->addWindowPopup(new WindowMessageWarning("WARNING", "Azy fait pas ieche"));

	if( p_event.type == sf::Event::KeyPressed && p_event.key.code == sf::Keyboard::D)
		UserInterface::mUserInterface->addWindowPopup(new WindowMessageError("ERROR", "Manouche !"));
}
